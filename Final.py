...
.stApp {
  background: linear-gradient(45deg, #020617 0%, #0f172a 40%, #1e293b 60%, #020617 100%);
  background-size: 200% 200%;
  animation: move-bg 20s ease infinite;
}

/* Optional: make main block slightly translucent “glass” */
...
if st.session_state.page == "Compare":
    ...
    if ds is None:
        ...
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("Story Mode: Guided Climate Tour")

        # Choose a variable that has time + lat + lon
        ...
        if not var_list:
            ...
            # Decide which time dim this variable uses
            if "time" in data_story.dims:
                time_dim_story = "time"
            else:
                time_dim_story = "TIME"
            ...
            # Map story_step to an index: start, middle, end
            step = st.session_state.story_step
            if step == 0:
                t_idx = 0
                title = "Baseline"
                desc = "Early period at the start of the record."
            elif step == 1:
                t_idx = nt // 2
                title = "Mid-period"
                desc = "Middle of the record."
            else:
                t_idx = nt - 1
                title = "Recent"
                desc = "Most recent time slice."
            ...
            # 2D map
            vmax_s = float(np.nanmax(np.abs(values_story)))
            fig_story = go.Figure(go.Contour(
                z=values_story, x=lon, y=lat,
                colorscale=palette,
                zmin=-vmax_s, zmax=vmax_s,
                contours=dict(coloring="heatmap", showlines=False),
            ...
            # 3D globe for this step (reusing your helper)
            fig_story_globe = make_globe_figure(
                lon=lon,
                lat=lat,
                values=values_story,
                title="3D Climate Globe (Story Step)",
            )
            st.plotly_chart(fig_story_globe, use_container_width=True)
            ...
            with col_prev:
                if st.button("⬅ Previous step", disabled=(step == 0)):
                    ...
                    st.rerun()
            with col_next:
                if st.button("Next step ➡", disabled=(step == 2)):
                    st.session_state.story_step = min(2, step + 1)
                    st.rerun()
